...
type (
	...
	OffersModel interface {
		...
		// FindByLocationIdAndDirection 根据位置ID、交易方向、箱型分类、箱况和颜色查询买卖交易挂单列表（支持游标分页）
		FindByLocationIdAndDirection(ctx context.Context, locationId int64, direction int64, category int64, condition int64, color int64, lastId int64, limit int64) ([]*Offers, error)
		// CountByLocationIdAndDirection 根据位置ID、交易方向、箱型分类、箱况和颜色统计符合条件的交易挂单总数
		CountByLocationIdAndDirection(ctx context.Context, locationId int64, direction int64, category int64, condition int64, color int64) (int64, error)
	}
